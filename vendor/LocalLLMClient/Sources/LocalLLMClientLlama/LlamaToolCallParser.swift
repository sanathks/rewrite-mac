...
struct LlamaToolCallParser {
    ...
    public static func parseToolCalls(from response: String, chatParams: UnsafeMutablePointer<llm_chat_params>?) -> [LLMToolCall]? {
        ...
        for i in 0..<parsedMessage.tool_calls.size() {
            ...
            // Swift 6.2 + Cxx interop: `String(std::string)` is ambiguous against the
            // `LosslessStringConvertible`-based init. Round-trip via C strings instead.
            let id = cppToolCall.id.empty() ? UUID().uuidString : String(cString: cppToolCall.id.__c_strUnsafe())
            let name = String(cString: cppToolCall.name.__c_strUnsafe())
            let arguments = String(cString: cppToolCall.arguments.__c_strUnsafe())
